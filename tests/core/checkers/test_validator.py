...
@pytest.mark.parametrize('validator_data, soft, current_data, expected_result', [
    ...
    [1, False, 1, None]])
def test_validator_positive(validator_data, soft, current_data, expected_result):
    validator = Validator(validator_data, soft=soft, ignore_extra_keys=False, report=Report(soft))
...
@pytest.mark.parametrize('validator_data, current_data, expected_result', (
    (
...
def test_validator_positive_message(validator_data, current_data, expected_result):
    soft = True
    validator = Validator(validator_data, soft=soft, report=Report(soft), ignore_extra_keys=False)
