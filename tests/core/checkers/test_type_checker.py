...
@pytest.mark.parametrize('type_data, soft, current_data, expected_result', [
    [int, True, 123, None],
    [int, False, 123, None],
    [int, False, True, None],
...
def test_type_checker_positive(type_data, soft, current_data, expected_result):
    type_checker = TypeChecker(expected_data=type_data, soft=soft, report=Report(soft=soft))
    assert type_checker.validate(current_data) == expected_result


@pytest.mark.parametrize('type_data, current_data', [
    [str, ["1"]],
    [type, 123],
    [str, [1]],
    [bool, 1],
...
def test_type_checker_negative(type_data, current_data):
    type_checker = TypeChecker(expected_data=type_data, soft=False, report=Report(soft=False))
