...
@pytest.mark.parametrize('list_data, soft, current_data', (
    ([1, 2], False, [1, 2]),
    ([int, int], False, [1, 2]),
    ([int, str], False, [1, 'x']),
    ([1, 2], True, [1, 2]),
...
def test_list_checker_positive(list_data, soft, current_data):
    list_checker = ListChecker(expected_data=list_data, soft=soft, report=Report(soft=soft))
    assert list_checker.validate(current_data) is None


@pytest.mark.parametrize('list_data, current_data, ex_message', (
    ([int], [1, '2', '3'], "current value '2' (str) is not int\n"
...
@pytest.mark.parametrize('list_data, current_data, ex_exception', [
